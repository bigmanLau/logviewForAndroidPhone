...
public class LogcatViewerService extends Service {
    ...
    public IBinder onBind(Intent intent) {
        mPackageName=intent.getComponent().getPackageName();
        return mBinder;
    }
    ...
    private void runLogcatSubscriber() {
        ...
        try {
            if(mPackageName!=null) {
                process = Runtime.getRuntime().exec("/system/bin/logcat |grep "+mPackageName);
            }else{
                process = Runtime.getRuntime().exec("/system/bin/logcat -v time");
            }
        } catch (IOException e) {
